...
impl Object for Pulsar {
    ...
    fn velocity(&mut self) -> Option<&mut Direction> {
        ...
        /*      check the code -- maybe it's feasible to return
                ...
                panic!("`Pulsar` like `Planet` have no `velocity`")
    }
}
...
fn main() {
    let mut objects: Vec<Box<dyn Object>> = vec![
        Box::new(Pulsar {
            ...
            weight: WeightPulsing { value: 100, counter: Default::default(), state: Pulse::On },
        }),
        Box::new(Asteroid {
